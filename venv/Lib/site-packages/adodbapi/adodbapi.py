...
def format_parameters(ADOparameters, show_value=False):
    ...
    try:
        if show_value:
            desc = [
                "Name: %s, Dir.: %s, Type: %s, Size: %s, Value: \"%s\", Precision: %s, NumericScale: %s"%(p.Name, adc.directions[p.Direction], adc.adTypeNames.get(p.Type, str(p.Type)+' (unknown type)'), p.Size, p.Value, p.Precision, p.NumericScale)
                for p in ADOparameters ]
        else:
            desc = [
                "Name: %s, Dir.: %s, Type: %s, Size: %s, Precision: %s, NumericScale: %s"%(p.Name, adc.directions[p.Direction], adc.adTypeNames.get(p.Type, str(p.Type)+' (unknown type)'), p.Size, p.Precision, p.NumericScale)
                for p in ADOparameters ]
        return '[' + '\n'.join(desc) + ']'
    except:
        return '[]'
...
##   allows the programmer to override the connection's default error handler
    ...
    def format_description(self,d):
        ...
        desc = "Name= %s, Type= %s, DispSize= %s, IntSize= %s, Precision= %s, Scale= %s NullOK=%s" % (d[0], adc.adTypeNames.get(d[1], str(d[1])+' (unknown type)'),
                d[2], d[3], d[4], d[5], d[6])
        return desc
    ...
    def get_returned_parameters(self):
        ...
        for p in tuple(self.cmd.Parameters):
            if verbose > 2:
                print('Returned=Name: %s, Dir.: %s, Type: %s, Size: %s, Value: "%s",' " Precision: %s, NumericScale: %s" % (p.Name, adc.directions[p.Direction],
