  ...
  def WriteEnumerationItems(self, stream):
    ...
    for name in names:
      ...
      if vdesc[4] == pythoncom.VAR_CONST:
        ...
        print("\t%-30s=%-10s # from enum %s" % (build.MakePublicAttributeName(name, True), use, enumName), file=stream)
        num += 1
    return num
...
class DispatchItem(build.DispatchItem, WritableItem):
    ...
    def WriteClassBody(self, generator):
        ...
        if specialItems["item"]:
            ...
            print('\t\treturn self._get_good_object_(self._oleobj_.Invoke(*(%d, LCID, %d, 1, key)), "Item", %s)' % (entry.desc[0], invoketype, resultCLSID), file=stream)
