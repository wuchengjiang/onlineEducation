...
class ArgFormatterFloat(ArgFormatter):
	...
	def GetBuildForGatewayPreCode(self):
		return "\tdbl%s = " % self.arg.name + self._IndirectPrefix( self._GetDeclaredIndirection(),
			0) + self.arg.name + ";\n"
	def GetParsePostCode(self):
		s = "\t"
		if self.gatewayMode:
...
class ArgFormatterShort(ArgFormatter):
	...
	def GetBuildForGatewayPreCode(self):
		return "\ti%s = " % self.arg.name + self._IndirectPrefix( self._GetDeclaredIndirection(),
			0) + self.arg.name + ";\n"
	def GetParsePostCode(self):
		s = "\t"
		if self.gatewayMode:
...
class ArgFormatterInterface(ArgFormatterPythonCOM):
	def GetInterfaceCppObjectInfo(self):
		return self.GetIndirectedArgName(1, self.arg.indirectionLevel), "%s * %s" % (self.GetUnconstType(), self.arg.name)
