	...
	def FindBuildSubItemEvents(self):
		...
		for item in range(numTypeInfos):
			...
			while 1:
				...
				funcflags = fdesc[9]
				try:
					isSubObject = not (funcflags & pythoncom.FUNCFLAG_FRESTRICTED) and funckind == pythoncom.FUNC_DISPATCH and invkind  == pythoncom.INVOKE_PROPERTYGET and elemdesc[0][0] == pythoncom.VT_PTR and elemdesc[0][1][0] == pythoncom.VT_USERDEFINED
				except:
					isSubObject = 0
				if isSubObject:
						try:
...
#		trace ("ParseScriptText", code[:20],"...", itemName, context, delimiter, sourceContextCookie, startLineNumber, flags, bWantResult)
		if bWantResult or self.scriptState == axscript.SCRIPTSTATE_STARTED or self.scriptState == axscript.SCRIPTSTATE_CONNECTED or self.scriptState == axscript.SCRIPTSTATE_DISCONNECTED :
...
#				print "ApplyInSS", codeBlock, fn, args
	...
	def HandleException(self, codeBlock):
		...
		exception = error.AXScriptException(self, codeBlock, exc_type, exc_value, exc_traceback)
